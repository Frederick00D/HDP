...
function table_normal(collection,collection_CI,collection_CI_u,mu,sigma,n,noise_K,ep_list)
    ...
    RowNames = {'Sample size';'K';'mu: Estimator';'mu: Variance';'mu: Coverage'; 'mu: Uncorrected Coverage';
        'sigma: Estimator';'sigma: Variance';'sigma: Coverage'; 'sigma: Uncorrected Coverage'};
    ...
    for k=1:length(ep_list)
        ...
        data_short = data(data >= q1 & data <= q9);
        estimator_mu = mean(data_short);
        estimator_mu_variance = var(data_short);

        data = collection(noise_K,2,:,k);
        q1 = quantile(data, 0.05);
        q9 = quantile(data, 0.95);
        data_short = data(data >= q1 & data <= q9);
        estimator_sigma = mean(data_short);
        estimator_sigma_variance = var(data_short);

        T(3,k) = round(estimator_mu, 3);
        T(4,k) = round(estimator_mu_variance, 3);
        ...
        T(8,k) = round(estimator_sigma_variance, 3);
...
function table_normal_contamination(collection,collection_CI,collection_CI_u,n,ep_list,contamination_percentage_list,mu,sigma)
    ...
    T([2,4],:) = squeeze(median(data_mle,2));
    for k = 1 : length(ep_list)
        ep = ep_list(k);
        data = squeeze(collection(:,:,k,:));
        T([5+6*k-5, 5+6*k-2],:)=squeeze(median(data,2));
        for j = 1:numCols
...
function [collection,Collection_CI,Collection_CI_u,TRAJ] = collection_normal(data,data_resample,method,S,h,p_n,noise_K,iteration_number,lam,ep_list,eta,theta_start)
    ...
    for k = 1:length(ep_list)
        ...
        if method == 1
            parfor i=1:S
                ...
                [hat_theta,CI,CI_u,traj] = simulation_pdp.private_gradient_normal(X,X_resample,h,p_n,noise_K,iteration_number,lam,ep,eta,theta_start);
        ...
        elseif method ==2
            parfor i=1:S
                X = data(:,i);
                X_resample=data_resample(:,i);
                [hat_theta,CI,CI_u,traj] = simulation_pdp.private_Nt_normal(X,X_resample,h,p_n,noise_K,iteration_number,lam,ep,eta,theta_start);
                collection(:,:,i,k) = hat_theta;
                TRAJ(:,i,k) = traj;
                Collection_CI(:,:,:,i,k) = CI;    
                Collection_CI_u(:,:,:,i,k) = CI_u;
...
function [collection,collection_CI,collection_CI_u] = collection_normal_contaminatin(data_contain,data_contain_resample,method,mu,sigma,n,S,h,p_n,noise_K,iteration_number,lam,ep_list,eta,theta_start, contamination_percentage_list)
        ...
        for k = 1:length(ep_list)   
            ...
            if method == 1
                parfor i=1:S
                    ...
                    [hat_theta,CI,CI_u,~] = simulation_pdp.private_gradient_normal(X,X_resample,h,p_n,noise_K,iteration_number,lam,ep,eta,theta_start);
            ...
            elseif method ==2
                parfor i=1:S
                    X = data_contain(:,i,j);
                    X_resample = data_contain_resample(:,i,j);
                    [hat_theta,CI,CI_u,~] = simulation_pdp.private_Nt_normal(X,X_resample,h,p_n,noise_K,iteration_number,lam,ep,eta,theta_start);
                    collection(:,i,k,j) = hat_theta(noise_K,:);
                    collection_CI(:,:,i,k,j) = CI(noise_K,:,:);
                    collection_CI_u(:,:,i,k,j) = CI_u(noise_K,:,:);
                end
...
function [hat_theta,CI,CI_u,traj] = private_gradient_normal(X,X_resample,h,p_n,noise_K,iteration_number,lam,ep,eta,theta_start)
    ...
    if ep>0 && ep<2
        f = @(e) OPT.privacy_composition_PDP(lam,e,noise_K)-ep;
        e = fzero(f,0.3);
        noise_sd = 2*sqrt(6)*sqrt( lam*(lam+1)/(2*log(1+lam*(lam+1)*e)) ) / (n^(1/p_n));
    else
        noise_sd = 0;
    end
    ...
    % For CI
    if ep>0 && ep<2
        f_CI = @(e) OPT.privacy_composition_PDP(lam,e,2)-ep;
        e = fzero(f_CI,0.3);
        noise_sd_W1 = sqrt(118) / (n^(1/p_n)) *sqrt( lam*(lam+1)/(2*log(1+lam*(lam+1)*e)) );
        noise_sd_W2 = ( 2*sqrt(6)/ (n^(1/p_n)) )^2 * sqrt( lam*(lam+1)/(2*log(1+lam*(lam+1)*e)) );
    else
        noise_sd_W1 = 0;
        noise_sd_W2 = 0;
    end
...
function [hat_theta,CI,CI_u,traj] = private_Nt_normal(X,X_resample,h,p_n,noise_K,iteration_number,lam,ep,eta,theta_start)
    % X is input data, shape (n,1)
    n = length(X);
    CI = ones(iteration_number,length(theta_start),2);
    CI_u = ones(iteration_number,length(theta_start),2);
    ...
    if ep>0 && ep<2
        f = @(e) OPT.privacy_composition_PDP(lam,e,noise_K)-ep;
        e = fzero(f,0.3);
        noise_sd_G = 2*sqrt(6) * sqrt( lam*(lam+1)/(2*log(1+lam*(lam+1)*e/2)) ) / (n^(1/p_n));
        noise_sd_H = sqrt(118) * sqrt( lam*(lam+1)/(2*log(1+lam*(lam+1)*e/2)) ) / (n^(1/p_n));
    else
        noise_sd_G = 0;
        noise_sd_H = 0;
    end
